...
(function (service) {
	Ti.API.info("GCM.JS SERVICE , HERE NOTIFICATION IS CREATED");
	var serviceIntent = service.getIntent();
	var message_notification = serviceIntent.hasExtra('message') ? serviceIntent.getStringExtra('message') : '';
	message_notification = JSON.parse(message_notification);
	
	var content = JSON.parse(message_notification.content);
	var title = (content.question ? content.question : ""),
	statusBarMessage = (content.question ? content.question : ""),
	message = "New Question";
	
	// var db = require("dbHelper");
	// db.saveQuestion(content);
	
	Ti.API.info(serviceIntent.getStringExtra('message') + " ");
	...
	launcherIntent.putExtra("payload", JSON.stringify(message_notification));
