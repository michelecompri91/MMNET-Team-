...
(function (service) {
	...
	launcherIntent = Ti.Android.createIntent({
		className: 'it.tn.uh.reply.ReplyActivity',
		action: Ti.Android.ACTION_SENDTO, // we need an action identifier to be able to track click on notifications
		packageName: Ti.App.id,
		flags: Ti.Android.FLAG_ACTIVITY_NEW_TASK | Ti.Android.FLAG_ACTIVITY_SINGLE_TOP
	});
	launcherIntent.addCategory(Ti.Android.CATEGORY_DEFAULT);
	launcherIntent.putExtra("ntfId", ntfId);
	launcherIntent.putExtra("payload", JSON.stringify({"title": title, "message": message}));
